...
final readonly class ProviderCollector
{
     ...
     */
    private function sortPackages(iterable $packages): array
    {
        ...
        foreach (array_keys($providerNames) as $name) {
            $dependencies[$name] = $this->findProviderDependencies($name, $byName, $providerNames);
        }
     ...
     * @return list<string>
     */
    private function findProviderDependencies(string $name, array $packages, array $providerNames): array
    {
        /** @var array<string, true> $visited */
        $visited = [$name => true];
        /** @var list<string> $queue */
        $queue = $packages[$name]->requires;
        ...
        for ($offset = 0; isset($queue[$offset]); $offset++) {
            ...
            foreach ($packages[$dependency]->requires as $transitiveDependency) {
